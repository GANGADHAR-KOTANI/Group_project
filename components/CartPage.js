  ...
  const recalc = () => {
    ...
    if (couponApplied) {
      if (sum < 500) {
        ...
        discount = 100;
        message = "🎉 You saved ₹100!";
      }
    }

    setCouponValue(discount);
    setCouponText(message);
    setFinalTotal(Math.max(0, sum - discount));
  };

  const handleApplyCoupon = () => {
    if (subtotal < 500) {
      Alert.alert("Minimum order ₹500 to apply coupon");
      return;
    }

    setCouponApplied(true);
    let discount = subtotal >= 1000 ? 200 : 100;
    let message = subtotal >= 1000 ? "🎉 You saved ₹200!" : "🎉 You saved ₹100!";
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 240 }}>
        ...
        {/* Coupon button appears dynamically */}
        {!couponApplied && subtotal >= 500 && (
      ...
      <View style={styles.bottomFixed}>
        ...
        <Text>Discount: ₹{couponValue}</Text>
      ...
      {couponText ? (
        ...
        >
          <Text style={{ color: "#fff" }}>{couponText}</Text>
        </Animated.View>
      ) : null}
    </View>
  );
